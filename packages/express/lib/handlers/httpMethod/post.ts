import { HttpMethodHandler, HttpResponseHandler, ControllerFactory, ResponseHandlerToken, RouterToken, ErrorHandler, GetOptions, PostMetadataKey } from "@nger/core";
...
const handler: HttpMethodHandler<any, GetOptions> = (controller: ControllerFactory<any>, decorator: IMethodDecorator<any, GetOptions>) => {
    if (decorator.options) {
        ...
        router.post(decorator.options.path, ...(decorator.options.useGuards || []).map(it => createGuard(controller, it)), (req, res, next) => {
...
export const postHandlerProvider: StaticProvider = {
